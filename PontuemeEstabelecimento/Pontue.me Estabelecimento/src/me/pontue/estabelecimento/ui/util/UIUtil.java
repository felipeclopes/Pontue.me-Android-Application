...
//		summarySubtitleText.setText(textStr.remove(0));
	...
	/*
	 * Metodo para percorrer e preencher o layout com os nextFocus entre cada EditText
	 */
	public static void fillRights(View rootLayout) {
		List<View> childs = new ArrayList<View>();
		getChilds(rootLayout, childs);
		List<EditText> texts = new ArrayList<EditText>();
		for (View v : childs) {
		...
		for (int i = 0; i < size; i++) {
			int id = texts.get(i + 1).getId();
			texts.get(i).setNextFocusRightId(id);
			texts.get(i).setNextFocusForwardId(id);
			texts.get(i).setNextFocusUpId(id);
			texts.get(i).setNextFocusDownId(id);
			texts.get(i).setNextFocusLeftId(id);
		}
	}
